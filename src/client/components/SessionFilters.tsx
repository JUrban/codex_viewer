...
export function SessionFilters({ filters, projects, onChange }: {
  ...
  return (
    <form className="filters" role="search" onSubmit={(event) => event.preventDefault()}>
      ...
      <input id="session-search" type="search" value={filters.q}
        onChange={(event) => patch({ q: event.target.value })}
        placeholder="Title, project, or message" />
      <div className="filter-grid">
        <label>Project
          <select value={filters.project} onChange={(event) => patch({ project: event.target.value })}>
