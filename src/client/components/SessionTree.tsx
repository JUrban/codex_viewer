...
const EMPTY_EXPANSION: ExpansionOverrides = {
  expanded: new Set(),
  collapsed: new Set(),
};
...
export function SessionTree({ entries, selectedId, onSelect, revealMatches = false }: {
  ...
  const [expansion, setExpansion] = useState(EMPTY_EXPANSION);

  useEffect(() => {
    setExpansion((current) => {
      if (![...selectedAncestors].some((id) => current.collapsed.has(id))) return current;
    ...
    <nav aria-label="Sessions">
      <ul className="session-list">
        {groups.map((group) => <SessionBranch key={group.root.session.id} group={group}
          selectedId={selectedId} selectedAncestors={selectedAncestors}
          expanded={expansion.expanded} collapsed={expansion.collapsed}
...
function SessionBranch({
  ...
  return <li>
    ...
    <div className="session-row">
      {hasChildren
        ? <button className={`session-disclosure${open ? " open" : ""}`} type="button"
            aria-expanded={open} aria-controls={childListId}
      ...
      <SessionButton entry={group.root} selected={selectedId === id}
        onSelect={onSelect} child={child} />
    </div>
    {open ? <ul className="child-list" id={childListId}>
      {group.children.map((nested) => <SessionBranch key={nested.root.session.id}
        group={nested} selectedId={selectedId} selectedAncestors={selectedAncestors}
        expanded={expanded} collapsed={collapsed} revealMatches={revealMatches}
        onSelect={onSelect} onToggle={onToggle} child />)}
    </ul> : null}
  </li>;
}
  ...
  return (
    <button className={`session${selected ? " selected" : ""}${child ? " child" : ""}`}
      ...
      <small className="session-meta-line">
        {child
          ? <>
              {agentLabels.map((label) => <span className="agent-label" key={label}>{label}</span>)}
